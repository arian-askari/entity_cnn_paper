...
%===================================
%For this task, we use the test collection introduced in \cite{Garigliotti:2017:TTI:3077136.3080659}. In this test collection, for a given query there is a pool of target entity types. for each target type in the pool, there is a label between zero (non-relevant) and seven (the most relevant) which indicates the relevance degree of the query and that target type.
%=================================== 
...
\boldsymbol{$I_{EC}$} \textbf{matrix:} For a given query-type pair $(q,t)$, we first used a language retrieval model to retrieve top-20 ranked entities (i.e $e_1,\cdots, e_{20}$) for query $q$.
